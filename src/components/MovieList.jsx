    ...
    <>
      {
        ...
        <>
          <Link to={`/movie/${data.id}`} className="lg:p-5 p-2 shrink-0 relative shadow-(--shadow)">
            <div
            ...
            >
              <img
                ...
                alt=""
                className={`lg:w-50 w-30 ${isHover && "scale-120"} rounded-2xl`}
              />
              {isHover && (
                <div className="absolute border bg-black/30 backdrop-blur-xs inset-0 rounded-2xl">
                  <div className="h-full flex flex-col gap-1 justify-end p-3">
                    <p className=" text-(--text) lg:font-bold font-extrabold text-xs lg:text-base">{data.title}</p>
